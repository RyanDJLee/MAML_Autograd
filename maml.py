...
def gd_step(cost, params, lrate):
    ...
    rate lrate. Returns a new set of parameters, and does not modify
    the input parameters."""
    grad_fun = ag.grad(cost)
    # Do not mutate the input parameters
    grads = grad_fun(params)
    x = {k: params[k] - lrate * grads[k] for k in params.keys()}
    return x
...
class MetaObjective:
    ...
    def __call__(self, params, return_traj=False):
        """Compute the meta-objective. If return_traj is True, return
        a list of the parameters after each update."""
        trajectory = [params]

        inner_obj = InnerObjective(self.x, self.y)
        # perform gradient descent for num_steps on the inner objective
        for i in range(self.num_steps):
            trajectory.append(gd_step(inner_obj, trajectory[i], self.inner_lrate))
        # compute the final cost; just use inner_obj because meta objective
        # is also mean squared error.
        final_cost = inner_obj(trajectory[-1])
        if return_traj:
...
def train():
    ...
    for i in range(OUTER_STEPS):
        ...
        # Instantiate meta objective and compute the gradient
        meta_obj = MetaObjective(x_train, y_train, INNER_LRATE, INNER_STEPS)
        params = gd_step(meta_obj, params, OUTER_LRATE)
        if (i+1) % PRINT_EVERY == 0:
            val_cost = MetaObjective(x_val, y_val, INNER_LRATE, INNER_STEPS)
            print('Iteration %d Meta-objective: %1.3f' % (i+1, val_cost(params)))
        #print('Outer cost:', meta_obj(params))
        if (i+1) % DISPLAY_EVERY == 0:
            meta_obj.visualize(params, 'Iteration %d' % (i+1), ax.flat[plot_id])
            plot_id += 1
    plt.savefig('out_final.png')
